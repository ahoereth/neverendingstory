...
Meteor.methods({
  ...
  'stories/voterNames':function(_id){
    check(_id, MeteorID);
    console.log("_idddddd "+_id);
    var votesBy = Stories.findOne(_id).votes;
    console.log("votesBy");
    console.log(votesBy);
    console.log(votesBy[0]);
  
    var who=votesBy[0];
    var aaa=  (Meteor.users.find({_id: votesBy[0]}).fetch())[0];
    console.log(aaa);
  //  return Meteor.users.findOne({_id:votesBy[0]}).username;
  ...
  'stories/watchNextElection': function() {
    ...
    function getChangeCountdown(story){
      return (-10000);
    }

    // Start new timeout.
    nextElectionTimeout = Meteor.setTimeout(function() {
      var electedParagraph = Components.findOne({story : story._id}, {sort: {priority : -1, voteCount : -1}});
      ...
      var changeCountdown = getChangeCountdown(story);
