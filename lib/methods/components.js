...
Meteor.methods({
  ...
  'paragraphs/new': function(args) {
    ...
    // fired on the server.
    if(!this.isSimulation && Components.find({'priority': args.priority}).count() == 2) {
   ...
   * Elects a component.
   *
   * TODO: This functionallity needs to move to a serverside observer which
   * always looks for stories which have an election date in the past.
   *
   * @param {String} _id
   */
