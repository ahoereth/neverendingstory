  ...
  'components/vote': function(_id) {
    ...
    if ( -1 === _.indexOf(component.votes, this.userId) ) {
      ...
      if(activityAlready.length==0){
          ...
          Activities.update({user_id:this.userId,target_id: _id,action_type: 4}, {$set:{deleted:false,createdAt:currentDate()}});
        }
    } else {
      ...
      Activities.update({user_id:this.userId,target_id: _id,action_type: 4}, {$set:{deleted:true,deletedAt:currentDate()}});
    }
  }
